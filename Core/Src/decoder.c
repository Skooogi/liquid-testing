...
void prvDetectPreamble(unsigned int sample)
{
	...
	else if ( (dr.last_sample == dr.lastlast_sample) && (sample != dr.last_sample) )	// If the two previous samples were equal, the current one should not be
	{
		...
		else																			// Full length of preamble reached
		{
			...
			dr.preamble_counter = 0;
			dr.decoding_in_progress = 1;												// Tells that the downmix frequency should be kept constant as long as decoding is in progress
		}
	...
	{
		dr.preamble_counter = 0;														// Start looking for a preamble from the start.
	}
	/* Update latest samples */
...
void prvDetectStartFlag(unsigned int sample)
{
	...
	else if ( dr.startflag_counter < (AIS_START_FLAG_LENGTH - 1) )			// Must check last sample of start flag separately, so stop iterating before the last one
	{
		if ( sample == dr.last_sample )
		{
			dr.startflag_counter++;
		}
		else																// Sample did not match the start flag pattern
		{
			dr.startflag_counter = 0;
			dr.preamble_found = 0;											// Reset preamble found flag, since we have to start looking for a preamble again.
			dr.decoding_in_progress = 0;									// Decoding is no longer in progress for this sample, can start looking from other channels as well
		}
	}
	...
	else																	// The sample did not fit the start flag pattern
	{
		/* Reset all necessary flags and counters as we start looking for messages all over again */
		dr.startflag_counter = 0;
		dr.preamble_found = 0;
		dr.decoding_in_progress = 0;
	}
...
void prvDetectEndFlag( unsigned int sample )
{
	...
	if ( (dr.encoded_payload_length < (AIS_MAX_ENCODED_PAYLOAD_LENGTH + 1)) || dr.endflag_counter )	// The end flag can start at latest after the maximum length of the payload and CRC. If the end flag counter is non-zero, don't stop, as the end flag can continue after the payload and CRC.
	{
		if ( dr.endflag_counter == 0 )											// The first symbol can be either 1 or 0.
		{
				dr.endflag_counter++;											// Found the first symbol (will always be found as it's value is allowed to be either 1 or 0)
		}
		...
		else if ( dr.endflag_counter == (AIS_END_FLAG_LENGTH - 1) )				// Check the last sample of the possible end flag
		{
			if ( sample != dr.last_sample )										// Sample fits end flag pattern
			{
				dr.endflag_counter = 0;											// Must be reset so that we don't continue in the outermost if-loop of this function
...
void prvPayloadAndCRCDecode()
{
	...
	uint32_t decoded_idx = 0;												// Current index of the decoded data buffer
	...
	for ( uint32_t i=1; i<dr.encoded_payload_length; i++ )					// Extract the rest of the payload (including CRC)
	{

		if ( dr.encoded_payload[i] == dr.encoded_payload[i-1] )
		{
			dr.decoded_payload[decoded_idx] = 1;							// Value same as previous --> found 1
			consecutive_ones++;
...
void prvPayloadTo6Bit()
{
	...
	for ( uint32_t i=0; i < dr.decoded_payload_length; i+=1 )
	{
		...
		{
			dr.dense_message_length++;									// Another byte has been filled
			dr.dense_message[dr.dense_message_length - 1] = byte;		// A byte is complete, so store it
			bit_idx = 0;
			byte = 0;
			if( dr.decoded_payload[i] == 1 ) byte |= 1 << (7-bit_idx);	// Bitshift to get the bits in their correct locations TODO: Verify this works correctly, especially for our 6 bit chars
		}
...
void prvExtractCRC()
{
	...
	/* Convert digitized CRC from array of "bits" to an uint16_t */
	uint8_t byte = 0;
	...
	dr.crc16 = (crc_array[0] << 8) | crc_array[1];					// Store CRC as one uint16_t value
}
