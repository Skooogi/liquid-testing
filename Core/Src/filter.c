...
{
    /* Options */
    filter.fc = 1.0*(ADC_SAMPLERATE/2.0)/(1.0*SYMBOLRATE * SAMPLES_PER_SYMBOL);			         		// Filter cutoff frequency
    filter.ft = 0.05f;         																			// Filter transition
    filter.attenuation = 60.0f;   																		// Stop-band attenuation [dB]
    filter.mu = 0.0f;          																			// Fractional timing offset

    // estimate required filter length and generate filter
    filter.num_taps = estimate_req_filter_len( filter.ft, filter.attenuation );							// Estimate filter length
    filter.taps = malloc( filter.num_taps * sizeof(float) );											// Remember to free at some point
    liquid_firdes_kaiser( filter.num_taps, filter.fc, filter.attenuation, filter.mu, filter.taps );		// Create filter taps and store them and their length

    filter.filter = firfilt_crcf_create( filter.taps, filter.num_taps );								// Create filter object
